  ...
  const handleDownloadReport = async (report: AdminReport) => {
    try {
      ...
      if ((report as any).documentId) {
        const documentId = (report as any).documentId;
        console.log(`Downloading document ${documentId} from form ${formId}`);
        
        // Get the original filename from the report or document
        let originalFilename = `report-${reportId}`;
        
        // Try to get filename from report documents array
